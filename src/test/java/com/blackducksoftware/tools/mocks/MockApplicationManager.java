...
public class MockApplicationManager implements IApplicationManager {
	...
	apps.add(new ApplicationPojo("app1", "app1", "Unspecified", null,
		ApprovalStatus.APPROVED, false, "ownerId"));
    }
	...
	for (int i = 0; i < 4; i++) {
	    ...
	    ApplicationPojo app = new ApplicationPojo(appName, appName, "v100",
		    null, ApprovalStatus.APPROVED, false, "testOwnerId");
	...
	ApplicationPojo app = new ApplicationPojo(name, name, version, null,
	...
	for (int i = 0; i < numberOfUsersToReturn; i++) {
	    ...
	    ApplicationUserPojo role = new ApplicationUserPojo("testAppName",
		    "Unspecified", appId, username, username,
		    "Application Developer", "appDev");
	    roles.add(role);
	}

	System.out.println("getAllUsersAssignedToApplication(" + appId + "): "
		+ roles.size());
	return roles;
    }
    ...
    public void addUsersByNameToApplicationTeam(String appId,
	    ...
	    throws CommonFrameworkException {
	System.out.println("addUsersByNameToApplicationTeam() called: " + appId
		+ ": " + userNames);
	recordOperation("add", appId, userNames);
    }

    private synchronized void recordOperation(String operationName,
	    String appId, Set<String> userNames) {
	String operation = operationName + ": " + appId + ": "
		+ asSortedList(userNames);
    ...
    @Override
    public void removeApplicationFromCacheById(String id)
	    throws CommonFrameworkException {
